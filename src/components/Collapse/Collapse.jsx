...
export default function Collapse(props) {
	...
	const [heightcoll, setHeightcoll] = useState(); // je definie le state de la hauteur du collapse
	...
	useEffect(() => {
		setHeightcoll(`${refHeight.current.scrollHeight}px`); //useEffect s'éxécute au montage du composant, dans ce cas, il définit la hauteur du collapse déplié lors de sa première ouverture et la conserve dans refHeight
	}, []);
	...
	return (
		...
		<div className={`collapse ${props.aboutStyle}`}>
			...
			<div
				...
				style={{ height: toggle ? `${heightcoll}` : "0px" }}
			>
